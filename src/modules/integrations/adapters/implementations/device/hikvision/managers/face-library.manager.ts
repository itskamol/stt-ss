...
export class HikvisionFaceLibraryManager {
    ...
    async createFaceLibrary(
        ...
        try {
            this.logger.debug('Creating face library', {
                deviceId: device.id,
                FDID: libraryData.FDID,
            });

            const response = await this.httpClient.request<ISAPIResponse & { FDID?: string }>(
                device,
                {
                    method: 'POST',
    ...
    async addFacePicture(
        ...
        try {
            this.logger.debug('Adding face picture', {
                deviceId: device.id,
            ...
            const response = await this.httpClient.request<
                ISAPIResponse & { FPID?: string; rowKey?: string }
            >(device, {
                method: 'POST',
    ...
    async applyFacePicture(
        ...
        try {
            this.logger.debug('Applying face picture', {
                deviceId: device.id,
                FDID: faceData.FDID,
            });
    ...
    async modifyFacePicture(
        ...
        try {
            this.logger.debug('Modifying face picture', {
                deviceId: device.id,
                FPID: faceData.FPID,
            });

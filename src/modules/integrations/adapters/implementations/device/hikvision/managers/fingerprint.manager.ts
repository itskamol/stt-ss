...
export class HikvisionFingerprintManager {
  ...
  async checkFingerprintSupport(device: any): Promise<boolean> {
    try {
      ...
      const isSupportFingerPrint = jsonResponse?.AccessControlCap?.isSupportFingerPrintCfg === 'true';
  ...
  async applyFingerprint(
    ...
    try {
      ...
      const response = await this.httpClient.request<ISAPIResponse & { employeeNo?: string; fingerPrintID?: string }>(device, {
        method: 'POST',

...
export class HikvisionSystemManager {
  ...
  async setUser(device: any, indexID: number, userInfo: UserInfo): Promise<ISAPIXMLResponse> {
    try {
      this.logger.debug('Setting user', { deviceId: device.id, indexID, userName: userInfo.userName });
  ...
  async getCardReaderConfig(device: any, cardReaderID: number): Promise<CardReaderCfg> {
    try {
      ...
      const response = await this.httpClient.request<{ CardReaderCfg: CardReaderCfg }>(device, {
        method: 'GET',
  ...
  async getCardVerificationRuleCapabilities(device: any): Promise<any> {
    try {
      this.logger.debug('Getting card verification rule capabilities', { deviceId: device.id });
   ...
   */
  createBasicCardReaderConfig(
    options: Partial<CardReaderCfg> = {}
  ): CardReaderCfg {
    return {
      enable: true,
   ...
   */
  createBasicWiegandConfig(
    options: Partial<WiegandCfg> = {}
  ): WiegandCfg {
    return {

...
export class HikvisionEventHostManager {
  ...
  async setAllListeningHosts(
    ...
    try {
      this.logger.debug('Setting all listening hosts', { deviceId: device.id, count: hostConfigs.length });
  ...
  async bulkConfigureListeningHosts(
    ...
    for (const config of hostConfigs) {
      try {
        ...
        this.logger.error('Failed to configure listening host in bulk operation', error.message, {
          deviceId: device.id,
          hostId: config.id,
        });
        results.push({

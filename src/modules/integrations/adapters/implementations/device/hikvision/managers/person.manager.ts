...
export class HikvisionPersonManager {
  ...
  async addPerson(
    device: any,
    personRequest: PersonAddRequest
  ): Promise<ISAPIResponse> {
    try {
      this.logger.debug('Adding person', { deviceId: device.id, employeeNo: personRequest.UserInfo.employeeNo });
  ...
  async modifyPerson(
    device: any,
    personRequest: PersonModifyRequest
  ): Promise<ISAPIResponse> {
    try {
      this.logger.debug('Modifying person', { deviceId: device.id, employeeNo: personRequest.UserInfo.employeeNo });
  ...
  async deletePersons(
    ...
    try {
      this.logger.debug('Deleting persons', { deviceId: device.id, mode: deleteRequest.UserInfoDetail.mode });
   ...
   */
  async searchPersonByEmployeeNo(
    device: any,
    employeeNo: string
